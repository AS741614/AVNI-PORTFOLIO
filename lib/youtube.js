...
// Strip hashtag spam, emoji, and stray separators from raw Shorts captions so
// titles read like headlines in cards, browser tabs, and search results.
// AI-generated seoTitles (lib/articles.js) still win when present.
...
// Same cleanup for card summaries: cut the trailing hashtag/keyword wall,
// then strip any remaining tags and emoji.
export function cleanSummary(text) {
  if (!text) return "";
  let t = String(text);
  const hashIdx = t.indexOf("#");
  if (hashIdx > 60) t = t.slice(0, hashIdx);
  return t
    .replace(/#[\p{L}\p{N}_]+/gu, " ")
    .replace(/[\p{Extended_Pictographic}\p{Regional_Indicator}\u{FE0F}\u{200D}]/gu, " ")
    .replace(/\s{2,}/g, " ")
...
function withArticle(story) {
  ...
  return {
    ...story,
    article,
    displayTitle: article?.seoTitle || cleanTitle(story.title) || story.title,
    summary: article?.dek || article?.metaDescription || cleanSummary(story.description),
  };
}

// Turn a YouTube title into a URL slug (cleaned first, so hashtags never leak in)
